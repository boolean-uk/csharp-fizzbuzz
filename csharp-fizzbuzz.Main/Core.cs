...
namespace csharp_fizzbuzz.Main
{
    public class Core
    {
        ...
        public String[] fizzbuzz(String[] nums)
        {
            ...
            * */
            int[] convertedList = new int[nums.Length];
            for (int i = 0; i < nums.Length; i++)
            {
                convertedList[i] = int.Parse(nums[i]);
            }

            for (int i = 0; i < convertedList.Length; i++)
            {
                if (convertedList[i] % 3 == 0 && convertedList[i] % 5 == 0)
                {
                    nums[i] = "Fizzbuzz";
                }
                else if (convertedList[i] % 3 == 0)
                {
                    nums[i] = "Fizz";
                }
                else if (convertedList[i] % 5 == 0)
                {
                    nums[i] = "Buzz";
                }
            }
